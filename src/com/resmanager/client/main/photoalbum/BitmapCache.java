	...
	public void displayBmp(final ImageView iv, final String thumbPath, final String sourcePath, final ImageCallback callback) {
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					if (isThumbPath) {
						thumb = Bimp.rotateBitmapByDegree(BitmapFactory.decodeFile(thumbPath), Bimp.getBitmapDegree(thumbPath));
						if (thumb == null) {
							thumb = Bimp.rotateBitmapByDegree(revitionImageSize(sourcePath), Bimp.getBitmapDegree(sourcePath));
						}
					} else {
						thumb = Bimp.rotateBitmapByDegree(revitionImageSize(sourcePath), Bimp.getBitmapDegree(sourcePath));
					}
	...
	public Bitmap revitionImageSize(String path) throws IOException {
		...
		while (true) {
			if ((options.outWidth >> i <= 256) && (options.outHeight >> i <= 256)) {
				...
				bitmap = Bimp.rotateBitmapByDegree(BitmapFactory.decodeStream(in, null, options), Bimp.getBitmapDegree(path));
				break;
			}
			i += 1;
		}
